...
# define MFC_DIALOG_MODULE_STATE
...
protected:
  ...
  NSImage* LoadIconResource(UINT iconResourceId, const ON_2iSize& sizeInPixels)
  {
    if (iconResourceId == 0)
      return nullptr;
//    MFC_DIALOG_MODULE_STATE;
//    CRhinoDpi::LoadIcon(::AfxGetInstanceHandle(), iconResourceId, sizeInPixels.cx, sizeInPixels.cy);
    return nullptr;
  }
private:
...
//  AFX_MODULE_STATE* DialogModuleState() const { return nullptr; }
  //---------------------------------------------------------------------------
#pragma region IRhinoWindow overrides
  //---------------------------------------------------------------------------
  bool Create(const CRhinoWindowCreateOptions& options) override
  {
    return BASECLASS::Create(m_dialog_resouce_id, nullptr) != false;
  }
  //---------------------------------------------------------------------------
  const wchar_t* EnglishCaption() const override { return L""; }
  //---------------------------------------------------------------------------
  const wchar_t* LocalCaption() const override { return L""; }
  //---------------------------------------------------------------------------
  ...
  //---------------------------------------------------------------------------
  void SetParent(NSView* parent) override
  {
//    if (Created())
//      ::SetParent(BASECLASS::m_hWnd, parent);
  }
  //---------------------------------------------------------------------------
  NSView* Parent() const override
  {
    return nullptr;
  ...
  {
    return ::IsWindow(BASECLASS::m_hWnd);
  }
  //---------------------------------------------------------------------------
...
//      BASECLASS::SetFocus();
  ...
  //---------------------------------------------------------------------------
  NSView* Handle(void) override
  {
    auto hwnd = BASECLASS::m_hWnd;
    return nullptr;
  }
  //---------------------------------------------------------------------------
...
#pragma region IRhinoPage overrides
  ...
  NSImage* Image(ON_2iSize sizeInPixels) override
  {
    return TRhinoDialogWindow<BASECLASS>::LoadIconResource(m_icon_resource_id, sizeInPixels);
  }
  //---------------------------------------------------------------------------
  // Description:
...
#define RHINO_PROPERTIES_PANEL_PAGE_DECLARE \
  bool IncludeInNavigationControl(IRhinoPropertiesPanelPageEventArgs& args) const override;\
  CRhinoCommand::result RunScript(CRhinoOptionsPageEventArgs& e) override;\
